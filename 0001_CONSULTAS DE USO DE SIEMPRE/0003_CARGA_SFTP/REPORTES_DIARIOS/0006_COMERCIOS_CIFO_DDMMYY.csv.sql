...
select soperacion as                                                       "OPERACIÓN",
       ...
       TO_CHAR(trunc(o.dtfechaoperacion), ' dd / MM / yyyy HH24:MI:SS ') "FECHA",
...
from COMERCIOCONSULTAS.operaciones o
    ...
    --left join comercio.operacionesdet opdet on coop.uidoperacion=opdet.uidoperacion
         LEFT JOIN (select co.scomercio, o.sfolioventa
                    from COMERCIOCONSULTAS.operaciones o
                             inner join comercio.comercios co on co.uidcomercio = o.uidcomercio
                             left join catalogos.estatus e on e.uidestatus = co.uidestatus
                    where o.DTFECHAOPERACION >= TRUNC(SYSDATE - [DESDE_MAS])
                      AND o.DTFECHAOPERACION < TRUNC(SYSDATE - [HASTA])
                      and co.snombregrupocomercial = 'Cajero' and co.scomercio like 'CIFO%' and o.sestatustransaccion = 'CANCELADA') X
                   ON x.scomercio = co.scomercio and x.sfolioventa = o.sfolioventa
