...
WITH
    BaseTarjetas as
        (
        select INUMEROTARJETA from CREDENCIALIZACION.TARJETAS where INUMEROTARJETA in (5000000000397172)
    ),
   SaldoLtcsExtra as (
       select t1.STARJETA,sum(t1.DSALDOACTUAL) SaldoAcumularLtcsFaltantes from BaseTarjetas BT
                join sincronizador.MONEDERO SM on BT.INUMEROTARJETA = SM.INUMEROTARJETA
                join sincronizador.CAMBIOSMONEDERO t1 on t1.STARJETA = SM.INUMEROTARJETA AND t1.ICONTADORRECARGA > SM.ICONTADORRECARGAAPLICADA AND T1.ICONTADORRECARGA <= SM.ICONTADORRECARGA
       group by t1.STARJETA)
--      select * from SaldoLtcsExtra;
    ,base AS (
    SELECT *
    FROM (
        SELECT
            ...
            t1.DSALDO SaldoTarjeta,
            t2.DSALDO AS SaldoMonedero,
              (((t1.DSALDO +  t4.SaldoAcumularLtcsFaltantes) * -1) + t2.DSALDO  ) AS SaldoDebitar,
              t4.SaldoAcumularLtcsFaltantes AS SaldoAbonoLtcsFaltantes,
--             (((t1.DSALDO +  t4.SaldoAcumularLtcsFaltantes) * -1) + t2.DSALDO  ) AS SaldoTarjetaAplicarDebitar,
            ROUND(  (((t1.DSALDO +  t4.SaldoAcumularLtcsFaltantes) * -1) + t2.DSALDO  ) / (CASE WHEN t2.STIPOTARIFA = 'Tarifa General' THEN 12 ELSE 5 END), 0) AS NUMDEBITACIONES,
            TRUNC(  (((t1.DSALDO +  t4.SaldoAcumularLtcsFaltantes) * -1) + t2.DSALDO  ) / (CASE WHEN t2.STIPOTARIFA = 'Tarifa General' THEN 12 ELSE 5 END)) AS PARTE_ENTERA,
            (  (((t1.DSALDO +  t4.SaldoAcumularLtcsFaltantes) * -1) + t2.DSALDO  ) / (CASE WHEN t2.STIPOTARIFA = 'Tarifa General' THEN 12 ELSE 5 END))
                - TRUNC(  (((t1.DSALDO +  t4.SaldoAcumularLtcsFaltantes) * -1) + t2.DSALDO  ) / (CASE WHEN t2.STIPOTARIFA = 'Tarifa General' THEN 12 ELSE 5 END)) AS PARTE_DECIMAL,
            t1.DTFECHAOPERACION,
        ...
        join SaldoLtcsExtra t4 on t4.STARJETA = t3.INUMEROTARJETA
        WHERE
            1=1
        --AND t3.ICONTADORRECARGA = t3.ICONTADORRECARGAAPLICADA
...
--SECCION DE REGISTROS ERROR MAS LOS REGISTROS BUENOS TOMANDO PRIMERO LOS DE ERROR Y LUEGO LOS BUENOS DE FECHAS INICIALES CON REGISTROS SIN ERROR
        ...
        SELECT
            ...
            TO_DATE('18-06-2025', 'DD-MM-YYYY') + (B.DTFECHAOPERACION - TRUNC(B.DTFECHAOPERACION)) AS NVAFECHAPROCESAR,
            CASE
