...
WITH BASE_INICIO AS (SELECT /*+SEBAS*/
...
--WHEN NFC.IERROR= 2 THEN 'Rechazada'
                         ...
                         CASE
                             ...
                             WHEN NFC.IERROR in (2, 3, 4, 6, 7) THEN
                                 ROW_NUMBER() OVER (PARTITION BY NFC.INUMEROTARJETA,TRUNC(NFC.DTFECHAOPERACION), CASE WHEN NFC.IERROR = 0 THEN 1 ELSE 0 END ORDER BY NFC.DTFECHAOPERACION ASC)
...
--                        AND TRUNC(NFC.DTFECHACREACION) <= TRUNC(SYSDATE))
                         AND TRUNC(NFC.DTFECHAOPERACION) >= (SYSDATE - 8)
                             AND TRUNC(NFC.DTFECHAOPERACION) < (SYSDATE-1)) --> SIEMPRE UN DIA ANTES
     ,
     baseMasSaldo
