...
--------------------------------------
INSERT INTO PREP_MONEDEROCOMANDOS.PAQUETES(UIDPAQUETE, SNOMBRE, IUNIDAD, UIDPRODUCTO)
SELECT T1.UIDPAQUETE,T1.SNOMBRE,T1.IUNIDAD,T1.UIDPRODUCTO FROM PREP_CATALOGOS.PAQUETES T1
JOIN PREP_CATALOGOS.PRODUCTOS T2
ON T1.UIDPRODUCTO = T2.UIDPRODUCTO
WHERE T1.BACTIVO = 1 AND T2.BAPP=1
AND (T1.UIDPAQUETE,T2.UIDPRODUCTO) NOT IN (
    SELECT UIDPAQUETE,UIDPRODUCTO
    FROM PREP_MONEDEROCOMANDOS.PAQUETES
);
--------------------------------------
...
SELECT * FROM PREP_CATALOGOS.PRODUCTOS WHERE BAPP=1;
SELECT * FROM PREP_MONEDEROCOMANDOS.PRODUCTOS ;

INSERT INTO PREP_MONEDEROCOMANDOS.PRODUCTOS(UIDPRODUCTO, SNOMBRE, SDESCRIPCION)
SELECT P.UIDPRODUCTO,P.SNOMBRE, P.SDESCRIPCION
FROM PREP_CATALOGOS.PRODUCTOS P
WHERE UIDPRODUCTO NOT IN (
    SELECT UIDPRODUCTO
    FROM PREP_MONEDEROCOMANDOS.PRODUCTOS
