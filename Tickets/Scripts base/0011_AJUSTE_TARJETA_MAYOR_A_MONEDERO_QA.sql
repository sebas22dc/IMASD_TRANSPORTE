...
-- CREATE SEQUENCE PREP_MONEDEROCOMANDOS.Seq_Foliador_Ajustes_Abonos
--   START WITH 10000
--   INCREMENT BY 1
--   NOCACHE
--   NOCYCLE;
...
CREATE GLOBAL TEMPORARY TABLE PREP_temp_registros_ajustar (
...
--  select * from PREP_temp_registros_ajustar;
 -- PASO 0.9 Limpiar la tabla temporal SIEMPRE POR SI ACASO
  delete from PREP_temp_registros_ajustar;
   -- Paso 1: AGREGAR A LA TALA TEMPORAL LOS DATOS QUE CORRESPONDAN 
   INSERT INTO PREP_temp_registros_ajustar 
...
lower(SUBSTR(RAWTOHEX(SYS_GUID()), 1, 8) || '-' ||
  ...
  NULL UIDUSUARIOBAJA,
 E.UIDMONEDERO,
 PREP_MONEDEROCOMANDOS.Seq_Foliador_Ajustes_Abonos.NEXTVAL IFOLIOMOVIMIENTO,
...
JOIN PREP_SINCRONIZADOR.MONEDERO SM on E.INUMTARJETA = SM.INUMEROTARJETA ---> directris de join para determinar que los ltcs son iguales
  WHERE 
 E.DSALDO < -12 
-- AND E.SESTATUS = 'ACTIVO'
  -- AND E.UIDMONEDERO = 'dc94aea1-d497-4ef9-b9fb-b16d8d9bc044'
 --AND E.INUMMONEDERO = 1000000001093128 --> cuando sea por lote esto se debe de quitar MUCHO CUIDADO CON ESTE DURANTE PRUEBAS, SIEMPRE DEBE TENER UN DATO DEL NUMERO DE MONEDERO
...
uidusuariobaja,uidmonedero,ifoliomovimiento,semisor,bfacturado,brecargado,bcancelado,uidtipooperacion from PREP_temp_registros_ajustar;
...
update PREP_temp_registros_ajustar T
set T.bInsertadoEstado = 1 
where(T.uidmovimientoestadodecuenta,T.uidoperacion)
  IN (select uidmovimientoestadodecuenta,uidoperacion from PREP_MONEDEROCOMANDOS.MOVIMIENTOSESTADOSDECUENTA where   BACTIVO=1 and BBAJA=0 );
...
null as sRuta,
null as sautobus
from PREP_temp_registros_ajustar me 
...
where T.uidoperacion
  IN (select uidoperacion from PREP_monederoconsultas.movimientos)
...
USING (
   ...
   uidmonedero,
   dmonto
   FROM PREP_temp_registros_ajustar
...
-----TABLA 2
MERGE INTO prep_monederoconsultas.estadodecuenta e
USING (
   SELECT 
   uidmonedero,
   dmonto
...
WHEN MATCHED THEN
   UPDATE SET e.DSALDO = e.DSALDO + datos.dmonto;
--TABLA 4
MERGE INTO prep_pagos.estadodecuenta e
USING (
   SELECT 
   uidmonedero,
   dmonto
